...
#############################################
##SOURCE: parosky git hub, contributors: ..## 
#############################################
...
def blend(img_target, img_source, img_mask, offset=(0, 0)):
	...
	for y in range(region_size[0]):
		for x in range(region_size[1]):
			#print "y: ", y, "x: ", x
			#print "IMG MASK SHAPE: ", img_mask.shape
			if img_mask[y,x]:
...
def test():
	img_mask = np.asarray( Image.open('./testimages/test1_mask.png'))
	img_mask.flags.writeable = True
	img_source = np.asarray(Image.open('./testimages/test1_src.png'))
	img_source.flags.writeable = True
	img_target = np.asarray(Image.open('./testimages/test1_target.png'))
